...
from keras_classification_models import get_submodules_from_kwargs
...
def ResNeXt_bifpn(
    ...
    for stage, rep in enumerate(model_params.repetitions):
        for block in range(rep):
            ...
            if stage == 0 and block == 2:
                features.append(x)
            elif stage == 1 and block == 3:
                features.append(x)
            elif stage == 2 and block == 5:
                features.append(x)
            elif stage == 3 and block == 1:
                features.append(x)
...
def ResNeXt101_Bifpn(input_shape=None, input_tensor=None, weights=None, classes=1000, include_top=True, **kwargs):
    return ResNeXt_bifpn(
        MODELS_PARAMS['resnext101'],
        input_shape=input_shape,
        input_tensor=input_tensor,
        include_top=include_top,

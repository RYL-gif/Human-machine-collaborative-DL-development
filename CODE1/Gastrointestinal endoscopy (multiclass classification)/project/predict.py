...
test_root = "../test dataset"  # test dataset    confirm
sub_folder_name = 'inner'
...
if not exter_no_label:
	...
	acc = (test_cm[7, 7] + test_cm[6, 6] + test_cm[5, 5] + test_cm[4, 4] + test_cm[3, 3] + test_cm[2, 2] + test_cm[1, 1] + test_cm[0, 0]) / np.sum(test_cm)
	print('ACC:' + str(acc))
	...
	if not os.path.exists(name):
		os.makedirs(name)
	if not os.path.exists(os.path.join(name, sub_folder_name)):
		os.makedirs(os.path.join(name, sub_folder_name))
	if not os.path.exists(os.path.join(name, sub_folder_name,'test_auc{}'.format(str(round(test_auc, 2))))):
		os.makedirs(os.path.join(name,sub_folder_name, 'test_auc{}'.format(str(round(test_auc, 2)))))
	with open(os.path.join(name, sub_folder_name,'test_auc{}'.format(str(round(test_auc, 2))), 'test_results.json'),
			  'w') as f:
		json.dump(test_result, f, indent=4)


	scores = pd.DataFrame(columns=['Image', 'Score0', 'Score1','Score2','Score3','Score4', 'Score5','Score6','Score7','Pred_Label'])
	scores['Image'] = files
	...
	scores['Score4'] = np.around(preds[:, 4], decimals=4)
	scores['Score5'] = np.around(preds[:, 5], decimals=4)
	scores['Score6'] = np.around(preds[:, 6], decimals=4)
	scores['Score7'] = np.around(preds[:, 7], decimals=4)
	scores['Pred_Label'] = pred_labels
	scores.to_csv(os.path.join(os.path.join(name, sub_folder_name, 'scores.txt')), sep=',',index=False)

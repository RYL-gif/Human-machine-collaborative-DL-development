...
model_name = 'inceptionresnetv2' # resnet50  inceptionv3  effientnetB4  inceptionresnetv2
...
if not exter_no_label:
	...
	# if not os.path.exists(os.path.join(name, 'innertest')):
	# 	os.makedirs(os.path.join(name, 'innertest'))

	scores = pd.DataFrame(columns=['Image', 'Score', 'Pred_Label'])
	scores['Image'] = files
	scores['Score'] = np.around(preds, decimals=4)
	scores['Pred_Label'] = pred_labels
	scores.to_csv(os.path.join(os.path.join(name, 'scores.txt')), sep=',',index=False)
	# scores.to_csv(os.path.join(os.path.join(name, 'innertest', 'scores.txt')), sep=',',index=False)
